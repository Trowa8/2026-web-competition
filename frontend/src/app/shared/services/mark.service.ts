...
export class MarkService {
    ...
    public async createMark(solutionId: string, body: CreateMarkRequest): Promise<MarkType> {
        return await firstValueFrom(this.http.post<MarkType>(`${environment.apiUrl}/marks/${solutionId}`, body));
    }

    public async getMarksBySolution(solutionId: string): Promise<MarkType[]> {
        return await firstValueFrom(this.http.get<MarkType[]>(`${environment.apiUrl}/marks/${solutionId}`));
    }
