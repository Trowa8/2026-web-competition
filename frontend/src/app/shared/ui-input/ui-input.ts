...
export class UiInputComponent {
    ...
    protected readonly isErrorVisible: Signal<boolean> = computed(() => this.error().length > 0);

    protected readonly currentStatus: Signal<InputStatus> = computed<InputStatus>(() =>
        this.isErrorVisible() ? "error" : this.status(),
    );

    protected readonly hasPrefixIcon: Signal<boolean> = computed(() => !!this.icon() || this.type() === "search");
    ...
    protected handleInput(event: Event): void {
        ...
        if (this.type() === "number") {
            val = val.replace(/[^0-9.]/g, "").replace(/^0+(?=\d)/, "");
